...
@Suite("DSCLI v2 flow and screen smoke")
struct V2FlowAndScreenCLISmokeTests {
    @Test("dsctl v2 validate-flow validates a checked-in v2 flow")
    func validateFlow() throws {
        let root = repositoryRoot()
        let result = try runCLI([
            "v2",
            "validate-flow",
            "--flow", root.appendingPathComponent("examples/v2/flows/commerce-checkout.flow.yaml").path,
            "--json",
        ])
    ...
    @Test("dsctl v2 validate-screen validates a checked-in v2 screen")
    ...
    @Test("dsctl v2 help exposes validate-flow and validate-screen")
    func v2Help() throws {
