...
@Suite("V2 sample app builds")
struct V2SampleAppsBuildTests {
    private let service = ProjectService()

    @Test("v2 app builds compile-backed sample app harnesses for ios and android")
    func buildSampleApps() throws {
        let fixture = try makeFixture()
        defer { try? FileManager.default.removeItem(at: fixture.root) }

        let report = try service.buildV2SampleApps(appPath: fixture.appURL)
    ...
    private func makeFixture() throws -> (root: URL, appURL: URL) {
        let root = URL(fileURLWithPath: NSTemporaryDirectory(), isDirectory: true)
            .appendingPathComponent("axys-v2-sample-app-tests-\(UUID().uuidString)", isDirectory: true)
