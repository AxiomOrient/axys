...
struct V2NativeManifest: Codable {
    let appId: String
    let flowId: String
    let screenId: String
...
struct V2NativeRenderer {
    ...
    private func renderSwiftScreen(context: V2ResolvedScreenContext) -> String {
    ...
    private func renderComposeScreen(context: V2ResolvedScreenContext) -> String {
        let screen = context.screenSpec
        let screenName = screenTypeName(from: screen.screenId)
        let stateName = "\(screenName)State"
        let actionsName = "\(screenName)Actions"
    ...
    private func renderSwiftStateFields(_ fields: [V2StateField]) -> String {
        if fields.isEmpty {
            return "    init() {}"
        }
    ...
    private func renderComposeStateFields(_ fields: [V2StateField]) -> String {
        if fields.isEmpty {
    ...
    private func renderSwiftActions(_ screen: V2ScreenSpec) -> String {
    ...
    private func renderComposeActions(_ screen: V2ScreenSpec) -> String {
        let fields = actionHandlerNames(screen: screen)
        if fields.isEmpty {
            return "    val unused: Unit = Unit"
        }
    ...
    private func renderSwiftNavigation(_ navigation: [V2Navigation]) -> String {
    ...
    private func renderComposeNavigation(_ navigation: [V2Navigation]) -> String {
        if navigation.isEmpty {
            return "    val unused: Unit = Unit"
        }
    ...
    private func renderSwiftNode(_ node: V2LayoutNode, context: V2ResolvedScreenContext, indent: Int) -> String {
    ...
    private func renderComposeNode(_ node: V2LayoutNode, context: V2ResolvedScreenContext, indent: Int) -> String {
        let prefix = String(repeating: "    ", count: indent)
        switch node.kind {
        case "stack", "slot", "conditional", "repeat", "grid":
    ...
    private func renderSwiftComponent(_ node: V2LayoutNode, context: V2ResolvedScreenContext, indent: Int) -> String {
        let prefix = String(repeating: "    ", count: indent)
    ...
    private func renderComposeComponent(_ node: V2LayoutNode, context: V2ResolvedScreenContext, indent: Int) -> String {
        let prefix = String(repeating: "    ", count: indent)
        guard let item = registryItem(for: node, in: context) else {
    ...
    private func swiftComponentArguments(node: V2LayoutNode, item: V2RegistryItem, screen: V2ScreenSpec) -> [String] {
        componentArguments(node: node, propMapping: item.native.ios.props, screen: screen, swiftEscaped: true).map { key, value in
            "\(key): \(value)"
        }
    }

    private func composeComponentArguments(node: V2LayoutNode, item: V2RegistryItem, screen: V2ScreenSpec) -> [String] {
        componentArguments(node: node, propMapping: item.native.android.props, screen: screen, swiftEscaped: false).map { key, value in
            "\(key) = \(value)"
        }
    }

    private func componentArguments(node: V2LayoutNode, propMapping: [String: String], screen: V2ScreenSpec, swiftEscaped: Bool) -> [(String, String)] {
    ...
    private func inferredPrimaryAction(for node: V2LayoutNode, screen: V2ScreenSpec) -> String? {
    ...
    private func inferredPrimaryNavigation(for node: V2LayoutNode, screen: V2ScreenSpec) -> String? {
    ...
    private func renderSwiftPreviewBlocks(screen: V2ScreenSpec, screenName: String, stateName: String, actionsName: String, navigationName: String) -> String {
    ...
    private func renderComposePreviewBlocks(screen: V2ScreenSpec, screenName: String, stateName: String, actionsName: String, navigationName: String) -> String {
        resolvedPreviewStates(for: screen).map { preview in
    ...
    private func actionHandlerNames(screen: V2ScreenSpec) -> [String] {
    ...
    private func collectBindings(in node: V2LayoutNode) -> [V2LayoutBinding] {
    ...
    private func resolvedPreviewStates(for screen: V2ScreenSpec) -> [V2PreviewState] {
        screen.previewStates.isEmpty ? [V2PreviewState(id: "default")] : screen.previewStates
    }

    private func swiftPreviewStateLiteral(_ preview: V2PreviewState, screen: V2ScreenSpec, stateName: String) -> String {
    ...
    private func kotlinPreviewStateLiteral(_ preview: V2PreviewState, screen: V2ScreenSpec, stateName: String) -> String {
        if screen.stateFields.isEmpty {
            return "\(stateName)()"
        }
    ...
    private func registryItem(for node: V2LayoutNode, in context: V2ResolvedScreenContext) -> V2RegistryItem? {
        guard let componentId = node.componentId else { return nil }
        return context.registry.items.first { $0.id == componentId }
    }

    private func layoutComment(for node: V2LayoutNode) -> String {
        switch node.kind {
        case "slot":
    ...
    private func motionComment(for screen: V2ScreenSpec) -> String {
    ...
    private func swiftLiteral(for field: V2StateField) -> String {
        swiftLiteral(for: field.type, value: field.defaultValue ?? defaultValue(for: field.type))
    }

    private func kotlinLiteral(for field: V2StateField) -> String {
