...
struct V2DocumentLoader {
    func load<T: Decodable>(_ type: T.Type, from url: URL) throws -> T {
        ...
        default:
            throw ProjectError.invalidArgument("Unsupported v2 contract format at \(url.path). Use .yaml, .yml, or .json")
        }
    }
}

struct V2ResolvedScreenContext {
    let contractRoot: URL
    let appURL: URL
    let appSpec: V2AppSpec
    let flowURL: URL
    let flowSpec: V2FlowSpec
    let screenURL: URL
    let screenSpec: V2ScreenSpec
    let registryURL: URL
    let registry: V2ComponentRegistry
    let motionURL: URL
    let motion: V2MotionSpec
    let reviewURL: URL
    let review: V2ReviewChecklist
}

public struct V2ProjectValidator {
    private let loader = V2DocumentLoader()
    private let fileManager = FileManager.default

    public init() {}

    public func validateApp(at appURL: URL) throws -> ValidationReport {
        let appSpec = try loader.load(V2AppSpec.self, from: appURL)
    ...
    public func validateFlow(at flowURL: URL) throws -> ValidationReport {
        ...
        let registry = loadReferenced(V2ComponentRegistry.self, at: registryURL, issuePath: "app.componentRegistry", original: appMatch.spec.componentRegistry, issues: &issues)
        let motion = loadReferenced(V2MotionSpec.self, at: motionURL, issuePath: "app.motionSet", original: appMatch.spec.motionSet, issues: &issues)
        let review = loadReferenced(V2ReviewChecklist.self, at: reviewURL, issuePath: "app.reviewSet", original: appMatch.spec.reviewSet, issues: &issues)
    ...
    func resolveScreenContext(at screenURL: URL) throws -> V2ResolvedScreenContext {
        let screenSpec = try loader.load(V2ScreenSpec.self, from: screenURL)
        let contractRoot = resolveContractRoot(startingAt: screenURL.deletingLastPathComponent())
        var issues: [ValidationIssue] = []
        let flowSpecs = try discoverFlowSpecs(contractRoot: contractRoot)
        ...
        return V2ResolvedScreenContext(
            contractRoot: contractRoot,
            appURL: appMatch.url,
    ...
    public func validate(_ appSpec: V2AppSpec, at appURL: URL) -> ValidationReport {
        ...
        let registry = loadReferenced(V2ComponentRegistry.self, at: registryURL, issuePath: "app.componentRegistry", original: appSpec.componentRegistry, issues: &issues)
        let motion = loadReferenced(V2MotionSpec.self, at: motionURL, issuePath: "app.motionSet", original: appSpec.motionSet, issues: &issues)
        let review = loadReferenced(V2ReviewChecklist.self, at: reviewURL, issuePath: "app.reviewSet", original: appSpec.reviewSet, issues: &issues)
        ...
        for (index, flowRef) in appSpec.flows.enumerated() {
            ...
            guard let flow = loadReferenced(V2FlowSpec.self, at: flowURL, issuePath: "app.flows[\(index)].path", original: flowRef.path, issues: &issues) else {
                continue
            }
    ...
    private func validateFlow(
        ...
        contractRoot: URL,
        registryItems: [String: V2RegistryItem],
        ...
        for (index, screenRef) in flow.screens.enumerated() {
            ...
            guard let screen = loadReferenced(V2ScreenSpec.self, at: screenURL, issuePath: "flow[\(flow.flowId)].screens[\(index)].path", original: screenRef.path, issues: &issues) else {
                continue
            }
    ...
    private func validateLayoutNode(
        ...
        path: String,
        screenURL: URL,
        stateFields: [String: V2StateField],
    ...
    private func validateBindings(
        _ bindings: [V2LayoutBinding],
        path: String,
        stateFields: [String: V2StateField],
        actionIDs: Set<String>,
        navigationIDs: Set<String>,
        component: V2RegistryItem,
        issues: inout [ValidationIssue]
    ) {
    ...
    private func validateRegistry(_ registry: V2ComponentRegistry?, issues: inout [ValidationIssue]) -> [String: V2RegistryItem] {
        ...
        var items: [String: V2RegistryItem] = [:]
    ...
    private func validateMotion(_ motion: V2MotionSpec?, issues: inout [ValidationIssue]) -> Set<String> {
    ...
    private func validateReview(_ review: V2ReviewChecklist?, issues: inout [ValidationIssue]) -> Set<String> {
    ...
    private func validateStateFields(_ fields: [V2StateField], path: String, issues: inout [ValidationIssue]) -> [String: V2StateField] {
        let allowedTypes: Set<String> = ["string", "boolean", "integer", "number", "enum"]
        var result: [String: V2StateField] = [:]
    ...
    private func validateActions(_ actions: [V2Action], path: String, issues: inout [ValidationIssue]) -> Set<String> {
    ...
    private func validateNavigation(_ navigation: [V2Navigation], path: String, issues: inout [ValidationIssue]) -> Set<String> {
        var result: Set<String> = []
    ...
    private func validateAssets(_ assets: [V2Asset], path: String, issues: inout [ValidationIssue]) -> Set<String> {
    ...
    private func validatePreviewStates(
        _ previewStates: [V2PreviewState],
        stateFields: [String: V2StateField],
        path: String,
        issues: inout [ValidationIssue]
    ) -> Set<String> {
    ...
    private func discoverAppSpecs(contractRoot: URL) throws -> [(url: URL, spec: V2AppSpec)] {
    ...
    private func discoverFlowSpecs(contractRoot: URL) throws -> [(url: URL, spec: V2FlowSpec)] {
        try discoverDocuments(
            contractRoot: contractRoot,
    ...
    private func resolveParentApp(
        for flowSpec: V2FlowSpec,
        flowURL: URL,
        appSpecs: [(url: URL, spec: V2AppSpec)],
        contractRoot: URL,
        issues: inout [ValidationIssue]
    ) -> (url: URL, spec: V2AppSpec)? {
    ...
    private func resolveParentFlow(
        for screenSpec: V2ScreenSpec,
        screenURL: URL,
        flowSpecs: [(url: URL, spec: V2FlowSpec)],
        contractRoot: URL,
        issues: inout [ValidationIssue]
    ) -> (url: URL, spec: V2FlowSpec)? {
